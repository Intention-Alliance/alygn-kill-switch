...
async function setupTestFile() {
	...
	await writeFile(TEST_SECRETS_PATH, JSON.stringify(secrets, null, 2), {
		mode: 0o600,
	});
...
describe("SecretsLoader", () => {
	...
	it("throws on missing required env var after file load", async () => {
		...
		await writeFile(
			TEST_SECRETS_PATH,
			JSON.stringify({ OTHER_TAILSCALE_KEY: "tsauth_val" }),
			{ mode: 0o600 },
		);
		await expect(loader.load()).rejects.toThrow(
			/FATAL.*OLLAMA_TAILSCALE_AUTH_TOKEN/,
		);
	});
	...
	it("loads *_TAILSCALE_* keys into process.env", async () => {
		await loader.load();
		expect(process.env.OLLAMA_TAILSCALE_AUTH_TOKEN).toBe(
			"tsauth_test_value_12345678",
		);
	...
	it("reloads when file content changes", async () => {
		...
		const newSecrets = {
			...makeTestSecrets(),
			OLLAMA_TAILSCALE_AUTH_TOKEN: "tsauth_NEW_value_99999",
		};
		await writeFile(TEST_SECRETS_PATH, JSON.stringify(newSecrets, null, 2), {
			mode: 0o600,
		});
	...
	it('emits "reloaded" event on successful reload', async () => {
		...
		const newSecrets = {
			...makeTestSecrets(),
			OLLAMA_TAILSCALE_AUTH_TOKEN: "tsauth_changed_val",
		};
		await writeFile(TEST_SECRETS_PATH, JSON.stringify(newSecrets, null, 2), {
			mode: 0o600,
		});
	...
	it("rotateKey generates a new server-side value and updates process.env", async () => {
		...
		const { maskedValue, rotatedAt } = await loader.rotateKey(
			"OLLAMA_TAILSCALE_AUTH_TOKEN",
		);
...
describe("SecretsLoader — rotateKey corrupt-JSON guard (Security #1)", () => {
	...
	it("refuses to rotate when secrets file is corrupt JSON", async () => {
		...
		await writeFile(TEST_SECRETS_PATH, "{ corrupt json {{{ broken", {
			mode: 0o600,
		});

		// rotateKey should throw, NOT silently start fresh
		await expect(
			loader.rotateKey("OLLAMA_TAILSCALE_AUTH_TOKEN"),
	...
	it("starts fresh when file does not exist (ENOENT is expected)", async () => {
		...
		const { maskedValue, rotatedAt } = await freshLoader.rotateKey(
			"OLLAMA_TAILSCALE_AUTH_TOKEN",
		);
		expect(maskedValue).toContain("••••");
		expect(rotatedAt).toBeTruthy();
