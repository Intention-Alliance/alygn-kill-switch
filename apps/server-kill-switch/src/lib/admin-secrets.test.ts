...
const TEST_DIR = resolve(
	tmpdir(),
	`api-secrets-test-${process.pid}-${Date.now()}`,
);
...
describe("/api/admin/secrets endpoints", () => {
	...
	it("returns 401 without Bearer token", async () => {
		...
		const handled = await handleAdminSecretsRoutes(
			"GET",
			"/api/admin/secrets",
			req,
			res,
	...
	it("lists all managed keys with masked values", async () => {
		...
		await handleAdminSecretsRoutes(
			"GET",
			"/api/admin/secrets",
			req,
			res,
	...
	it("rotates a key and returns masked value", async () => {
		const req = makeReq(
			"POST",
			"/api/admin/secrets/OLLAMA_TAILSCALE_AUTH_TOKEN/rotate",
			"",
			TEST_API_KEY,
		);
		const res = makeRes();
	...
	it("rejects rotation of keys that are not loaded/managed", async () => {
		const req = makeReq(
			"POST",
			"/api/admin/secrets/SOME_OTHER_KEY/rotate",
			"",
			TEST_API_KEY,
		);
		const res = makeRes();
	...
	it("returns lockout state for a key", async () => {
		const req = makeReq(
			"GET",
			"/api/admin/secrets/OLLAMA_TAILSCALE_AUTH_TOKEN/lockout",
			undefined,
			TEST_API_KEY,
		);
		const res = makeRes();
	...
	it("supports filtering audit log by keyName", async () => {
		...
		const req = makeReq(
			"GET",
			"/api/admin/secrets/audit?name=KEY_A",
			undefined,
			TEST_API_KEY,
		);
		const res = makeRes();
