...
import { closeSync, type FSWatcher, fsyncSync, openSync, watch } from "node:fs";
...
export class LockoutStateMachine extends EventEmitter {
	...
	async load(): Promise<void> {
		try {
			...
			if (
				...
				typeof parsed.consecutive401s === "number"
			) {
	...
	async record401(sourceIp: string): Promise<LockoutCheckResult> {
		...
		// Check threshold
		if (
			this.state.consecutive401s >= THRESHOLD_401_COUNT &&
