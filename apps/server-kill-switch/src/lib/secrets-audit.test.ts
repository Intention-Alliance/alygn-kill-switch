...
const TEST_DIR = resolve(
	tmpdir(),
	`secrets-audit-test-${process.pid}-${Date.now()}`,
);
...
async function _countDbEntriesByAction(event: string): Promise<number> {
	const rows = await db
		...
		.where(require("drizzle-orm").eq(secretsAuditLog.event, event));
	return rows.length;
}
...
describe("Secrets Audit Log — DB Persistence (S-A1)", () => {
	...
	it("rotate event writes to DB", async () => {
		...
		const rotateRows = await db
			.select()
			.from(secretsAuditLog)
			.where(eq(secretsAuditLog.event, "rotate"));
	...
	it("401 event writes to DB", async () => {
		...
		const rows = await db
			.select()
			.from(secretsAuditLog)
			.where(eq(secretsAuditLog.event, "401"));
