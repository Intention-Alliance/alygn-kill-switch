...
import { featureFlags, machineFlags, machines, settings } from "./schema";
...
export async function seedDefaults() {
	...
	const existingMachine = await db
		...
		.from(machines)
		.where(
			eq(machines.hostname, process.env.ALYGN_MACHINE_HOSTNAME ?? "localhost"),
		)
		.get();
	...
	if (!existingMachine) {
		await db.insert(machines).values({
			...
			name:
				process.env.ALYGN_MACHINE_NAME ??
				(process.env.ALYGN_MACHINE_HOSTNAME ?? "localhost").split(".")[0],
	...
	for (const [legacyKey, canonicalKey] of Object.entries(LEGACY_FLAG_KEY_MAP)) {
		...
		if (canonicalRow) {
			...
			await db
				...
				.run();
			console.log(
				`[seed] Flag key reconciled: removed legacy "${legacyKey}" (canonical "${canonicalKey}" already exists)`,
			);
		} else {
			await db
				.update(featureFlags)
				...
				.run();
			console.log(
				`[seed] Flag key reconciled: "${legacyKey}" → "${canonicalKey}"`,
			);
		}
		...
		for (const ov of legacyOverrides) {
			const canonicalOv = await db
				...
				.where(
					and(
						eq(machineFlags.machineId, ov.machineId),
						eq(machineFlags.flagKey, canonicalKey),
					),
				)
				.get();
			if (canonicalOv) {
				await db
					...
					.where(
						and(
							eq(machineFlags.machineId, ov.machineId),
							eq(machineFlags.flagKey, legacyKey),
						),
					)
					.run();
			} else {
