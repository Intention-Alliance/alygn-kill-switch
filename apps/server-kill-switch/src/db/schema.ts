...
export const killSwitchAuditLog = sqliteTable(
	...
	(table) => ({
		severityTimeIdx: index("ks_audit_severity_time_idx").on(
			table.severity,
			table.timestamp,
		),
		machineTimeIdx: index("ks_audit_machine_time_idx").on(
			table.machineId,
			table.timestamp,
		),
		stateTimeIdx: index("ks_audit_state_time_idx").on(
			table.newState,
			table.timestamp,
		),
...
export const flagAuditLog = sqliteTable(
	...
	(table) => ({
		...
		actionTimeIdx: index("flag_audit_action_time_idx").on(
			table.action,
			table.timestamp,
		),
...
export const machines = sqliteTable(
	"machine",
	{
		...
		monitoringOnly: integer("monitoring_only", { mode: "boolean" })
			.notNull()
			.default(true),
...
export const secretsAuditLog = sqliteTable(
	...
	(table) => ({
		...
		eventTimeIdx: index("secrets_audit_event_time_idx").on(
			table.event,
			table.at,
		),
	}),
...
export const webhookApiKeys = sqliteTable(
	...
	(table) => ({
		apiKeyHashUnique: uniqueIndex("webhook_api_keys_apiKeyHash_unique").on(
			table.apiKeyHash,
		),
		prefixIdx: index("webhook_api_keys_prefix_idx").on(table.keyPrefix),
		activeIdx: index("webhook_api_keys_active_idx").on(
			table.revokedAt,
			table.expiresAt,
		),
	}),
...
export const webhookApiKeyAudit = sqliteTable(
	...
	(table) => ({
		...
		actionAtIdx: index("webhook_api_key_audit_action_at_idx").on(
			table.action,
			table.at,
		),
	}),
...
export const registrationRequests = sqliteTable(
	...
	(table) => ({
		...
		createdAtIdx: index("registration_request_created_at_idx").on(
			table.createdAt,
		),
	}),
);
...
export const rogueDeviceAlerts = sqliteTable(
	"rogue_device_alert",
	{
		...
		lastDeniedAt: integer("last_denied_at", { mode: "timestamp" })
			.notNull()
			.$defaultFn(() => new Date()),
...
export const webauthnCredentials = sqliteTable(
	...
	(table) => ({
		...
		credentialIdUnique: uniqueIndex(
			"webauthn_credential_credential_id_unique",
...
export const killAuthorizationRequests = sqliteTable(
	...
	(table) => ({
		...
		initiatedAtIdx: index("kill_authorization_request_initiated_at_idx").on(
			table.initiatedAt,
		),
...
export const webhookKeys = sqliteTable(
	...
	(table) => ({
		hashedSecretUnique: uniqueIndex("webhook_keys_hashed_secret_unique").on(
			table.hashedSecret,
		),
...
export const firstAccess = sqliteTable(
	...
	(table) => ({
		keyIpDeviceIdx: uniqueIndex("first_access_key_ip_device_unique").on(
			table.keyId,
			table.ip,
			table.deviceFp,
		),
...
export const verificationEvents = sqliteTable(
	"verification_event",
	{
		...
		triggeredKill: integer("triggered_kill", { mode: "boolean" })
			.notNull()
			.default(false),
		createdAt: integer("created_at", { mode: "timestamp" })
			.notNull()
