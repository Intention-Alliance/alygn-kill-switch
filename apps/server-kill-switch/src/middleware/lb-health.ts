...
function formatPrometheus(metrics: ReturnType<typeof getMetrics>): string {
	...
	lines.push(
		`# HELP kill_switch_errors_total Total number of HTTP errors (5xx)`,
	);
	lines.push(`# TYPE kill_switch_errors_total counter`);
	lines.push(`kill_switch_errors_total ${metrics.errorCount}`);

	lines.push(
		`# HELP kill_switch_response_time_ms Response time in milliseconds`,
	);
	lines.push(`# TYPE kill_switch_response_time_ms summary`);
	lines.push(
		`kill_switch_response_time_ms{quantile="0.5"} ${metrics.responseTimeP50}`,
	);
	lines.push(
		`kill_switch_response_time_ms{quantile="0.9"} ${metrics.responseTimeP90}`,
	);
	lines.push(
		`kill_switch_response_time_ms{quantile="0.99"} ${metrics.responseTimeP99}`,
	);
	lines.push(`kill_switch_response_time_ms_avg ${metrics.responseTimeAvg}`);

	lines.push(
		`# HELP kill_switch_active_connections Current active connections`,
	);
