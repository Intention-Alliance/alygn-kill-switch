	...
	const fakeService: VerificationService = {
		...
		handleInferenceRequest: (opts) => {
			...
			return Promise.resolve({
				...
				// biome-ignore lint/correctness/noConstantCondition: deliberate always-false gate — keeps the sync-mode branch documented without enabling it.
				...(false ? { result: r } : {}),
			});
		},
...
async function runFlow(opts: {
	...
	// 1. Gate
	const gate: InferenceGateDecision = checkInferenceGate(
		"POST",
	...
	if (gate.gated) {
		return {
			...
			gate,
			verifierCalls: [],
			response: null as null | { status: number; body: string },
		};
	}
	...
	const relay = await relayInferenceRequest(
		makeReq(opts.body, opts.headers),
		"/v1/inference/generate",
		{
...
describe("deriveFingerprint (priority order)", () => {
	...
	it("priority 3: x-fingerprint header", () => {
		expect(deriveFingerprint({ headers: { "x-fingerprint": "fp-1" } })).toBe(
			"fp:fp-1",
		);
	});
