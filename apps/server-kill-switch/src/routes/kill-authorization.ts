...
export function createKillAuthorizationExecutor(
	service: KillSwitchService,
): KillAuthorizationExecutor {
	return {
	...
	try {
		...
		if (method === "POST" && url === "/v1/kill-authorization/requests") {
			...
			if (!body?.target || typeof body.target !== "string") {
				json(res, 400, {
					error: 'Missing required field: target (machine id or "fleet")',
				});
				return true;
			}
		...
		const approveMatch = url.match(
			/^\/v1\/kill-authorization\/requests\/([^/]+)\/approve$/,
		);
