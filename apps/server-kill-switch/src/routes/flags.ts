 ...
 */
async function assertKillAuthorizationFlagChangeAllowed(
	flagKey: string,
): Promise<void> {
...
export async function handleFlagsRoutes(
	...
	if (
		!url.startsWith("/v1/flags") &&
		!url.match(/^\/v1\/machines\/[^/]+\/flags(\/|$)/)
	) {
		return false;
	}
	...
	try {
		...
		if (method === "POST" && url === "/v1/flags") {
			...
			json(res, 201, {
				flag: {
					...flag,
					value: coerceFlagValue(flag.value, type),
			...
			if (publishEvent) {
				await publishEvent(
					"bcp:flags:updates",
					JSON.stringify({ type: "flag-update", action: "created", flag }),
				);
			}
			return true;
		}
		...
		if (method === "PUT" && idMatch) {
			...
			try {
				...
				json(res, gateErr.statusCode || 403, {
		...
		if (method === "DELETE" && idMatch) {
			...
			await db
				.delete(machineFlags)
				.where(eq(machineFlags.flagKey, existing.key))
				.run();
			...
			if (publishEvent) {
				await publishEvent(
					"bcp:flags:updates",
					JSON.stringify({ type: "flag-update", action: "deleted", flagId }),
				);
			}
			return true;
		}
		...
		if (method === "GET" && machineFlagsListMatch) {
			...
			const emitFlag = (
				key: string,
				global: (typeof globals)[number] | undefined,
				ov: (typeof overrides)[number] | undefined,
			) => {
				...
				if (ov) {
					...
					rawResolved =
						global.value === null || global.value === undefined
							? null
							: String(global.value);
				} else {
					rawResolved = null;
				...
				if (coerced === null) {
					...
					console.warn(
						`[flags] Unparseable value for ${key}: ${rawResolved} — surfacing raw`,
					);
					flags.push({
						key,
						value: rawResolved,
				...
				overrides: overrides.map((o) => ({
					...
					updatedAt:
						o.updatedAt instanceof Date
							...
							: new Date(o.updatedAt as any).toISOString(),
				})),
			});
			return true;
		}
		...
		const machineFlagKeyMatch = url.match(
			/^\/v1\/machines\/([^/]+)\/flags\/([^/]+)$/,
		);
		if ((method === "PUT" || method === "DELETE") && machineFlagKeyMatch) {
			...
			if (method === "PUT") {
				...
				if (
					flagKey === "auto_stop_threshold" ||
					flagKey === "request_sampling_rate"
				) {
					const n = Number(stored);
				...
				const prior = await db
					...
					.where(
						and(
							...
							eq(machineFlags.flagKey, flagKey),
						),
					)
					.get();
