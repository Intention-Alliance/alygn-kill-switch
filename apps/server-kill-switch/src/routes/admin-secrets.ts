...
export async function initAuditLogFromDb(): Promise<void> {
	...
	try {
		...
		console.log(
			`[admin-secrets] Loaded ${auditLog.length} audit entries from DB`,
		);
	} catch (err) {
...
export async function appendAuditEntry(
	entry: Omit<SecretsAuditEntry, "id">,
): Promise<SecretsAuditEntry> {
...
export async function handleAdminSecretsRoutes(
	...
	if (method === "POST" && rotateMatch) {
		...
		if (!secretsLoader.getLoadedKeys().includes(keyName)) {
			...
			res.end(
				JSON.stringify({
					error: `Key '${keyName}' is not a managed secret key`,
				}),
			);
			return true;
		}
		...
		try {
			...
			await appendAuditEntry({
				...
				result: "ok",
				meta: {
					consumersWritten: writeResults.filter((r) => r.result === "ok")
						.length,
				},
			});
			...
			for (const wr of writeResults) {
				await appendAuditEntry({
					...
					result: wr.result,
					meta: {
						consumer: wr.consumer,
						reloaded: wr.reloaded,
						error: wr.error || wr.reloadError,
					},
				});
			}
	...
	if (method === "GET" && lockoutMatch) {
		...
		res.end(
			JSON.stringify({
				name: keyName,
				state: check.state,
				lastEventAt: check.lastEventAt
					? new Date(check.lastEventAt).toISOString()
					: null,
				autoUnlockAt: check.autoUnlockAt
					? new Date(check.autoUnlockAt).toISOString()
					: null,
	...
	if (method === "GET" && url.startsWith("/api/admin/secrets/audit")) {
		...
		const keyName =
			...
			parsedUrl.searchParams.get("keyName") ||
			undefined;
		const event =
			parsedUrl.searchParams.get("event") ||
