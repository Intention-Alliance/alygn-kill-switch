...
function validateName(name: string): string | null {
	...
	if (!/^[a-zA-Z0-9-]+$/.test(name))
		return "name must be alphanumeric + hyphens only";
	return null;
}
...
function validateRole(role: string): string | null {
	...
	if (role.length < 1 || role.length > 32)
		return "role must be 1-32 characters";
	return null;
}
...
function serializeMachine(row: any): Machine {
	...
	const heartbeatTimeoutMs = Number(
		process.env.KILL_SWITCH_HEARTBEAT_TIMEOUT_MS ?? 90_000,
	);
	const lastSeenMs = row.lastSeen ? new Date(row.lastSeen).getTime() : null;
	const connected =
		lastSeenMs !== null && Date.now() - lastSeenMs <= heartbeatTimeoutMs;
	...
	return {
		...
		monitoringOnly:
			row.monitoringOnly !== undefined
				? Boolean(row.monitoringOnly)
				: undefined,
...
export async function handleMachinesRoutes(
	...
	try {
		...
		if (
			method === "GET" &&
			(url === "/v1/machines" || url === "/v1/machines/")
		) {
			...
			if (statusFilter === "pending") {
				...
				statusFilter &&
				["active", "inactive", "offline"].includes(statusFilter)
			) {
				...
				if (agentRow) {
					...
					await db.insert(agents).values({
						...
						capabilities: body.agentCapabilities
							? JSON.stringify(body.agentCapabilities)
							: null,
						lastHeartbeat: now,
						createdAt: now,
					});
		...
		if (method === "GET" && statusMatch) {
			...
			const agentList: AgentInfo[] = machineAgents.map((a: any) => ({
				...
				lastHeartbeat: a.lastHeartbeat
					? new Date(a.lastHeartbeat).toISOString()
					: null,
			...
			// Get global flags
			const globalFlags = await db.select().from(featureFlags).all();
			...
			json(res, 200, {
				machine: {
					...
					lastSeen: machine.lastSeen
						? new Date(machine.lastSeen).toISOString()
						: null,
				},
				dpu,
				agents: agentList,
		...
		if (method === "GET" && auditMatch) {
			...
			const logs = await db
				...
				.where(
					or(
						eq(killSwitchAuditLog.machineId, id),
						isNull(killSwitchAuditLog.machineId),
					),
				)
		...
		if (method === "PATCH" && idMatch) {
			...
			if (body.status !== undefined) {
				if (
					!["active", "inactive", "offline", "pending"].includes(body.status)
				) {
					json(res, 400, {
						error: "status must be active, inactive, offline, or pending",
					});
					return true;
				}
