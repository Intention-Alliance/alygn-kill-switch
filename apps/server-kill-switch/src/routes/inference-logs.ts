	...
	try {
		...
		if (method === "GET" && pathname === "/v1/inference-logs") {
			...
			const limit = Math.min(
				parseInt(urlObj.searchParams.get("limit") ?? "50", 10) || 50,
				200,
			);
			const offset = Math.max(
				parseInt(urlObj.searchParams.get("offset") ?? "0", 10) || 0,
				0,
			);
			...
			const rows = machineId
				? await base
						.where(eq(inferenceLogs.machineId, machineId))
						.orderBy(desc(inferenceLogs.timestamp))
						.limit(limit)
						.offset(offset)
						.all()
				: await base
		...
		if (method === "POST" && url === "/v1/inference-logs") {
			...
			const entry = {
				...
				timestamp: new Date(),
				machineId:
					typeof body.machineId === "string" ? body.machineId : "unknown",
				...
				promptPreview:
					typeof body.promptPreview === "string"
						? body.promptPreview.slice(0, 200)
						: null,
