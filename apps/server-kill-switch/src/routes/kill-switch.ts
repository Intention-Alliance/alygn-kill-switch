...
function requireKillAssertion(
	...
	try {
		return verifyAssertionTokenForAction({
			token,
			action: killActionForTarget(target),
		});
	} catch (err: any) {
...
export async function handleKillSwitchRoutes(
	...
	try {
		...
		if (method === "POST" && url === "/v1/kill-switch/chaos") {
			...
			const target =
				body.target && typeof body.target === "string" ? body.target : "fleet";
			...
			if (assertion) {
				...
				if (!ar.authenticated || ar.user?.role !== "admin") {
					...
					res.end(
						JSON.stringify({
							error:
								"Kill authorization requires a WebAuthn assertion token or an authenticated admin session",
