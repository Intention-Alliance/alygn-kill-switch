...
export async function handleInternalKillSwitchRoutes(
	...
	if (!internalKeyMatches(req)) {
		...
		writeJson(res, hasInternalCredential(req) ? 403 : 401, {
			error: "unauthorized",
		});
		return true;
	}
	...
	if (
		...
		!ALLOWED_AUTOMATED_STATES.includes(target as KillSwitchState)
	) {
		writeJson(res, 400, {
	...
	const initiatedBy =
		typeof body.initiatedBy === "string" &&
		body.initiatedBy.startsWith("system:")
			? body.initiatedBy
			: "system:verifier";
	const reason =
		typeof body.reason === "string" ? body.reason : "automated-stop";
	const machineId =
	...
	try {
		...
		console.error(
			"[internal-kill-switch] Transition failed:",
