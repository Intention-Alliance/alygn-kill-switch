			...
			values(data: any) {
				...
				return {
					...
					onConflictDoUpdate() {
						...
						const result: any = {
							run: execute,
							// biome-ignore lint/suspicious/noThenProperty: intentional thenable mock — the route `await`s this insert builder.
							then(resolve: any) {
								execute();
								return Promise.resolve(
									resolve ? resolve(undefined) : undefined,
								);
							},
						};
						return result;
...
describe("POST /v1/kill-authorization/requests — initiate kill", () => {
	it("single mode: assertion token → 200 EXECUTED", async () => {
		...
		const handled = await handleKillAuthorizationRoutes(
			"POST",
			"/v1/kill-authorization/requests",
			req,
			res,
			mockService,
		);
...
describe("POST /v1/kill-authorization/requests/:id/approve — quorum approval", () => {
	it("quorum: second distinct human approves → 200 EXECUTED", async () => {
		...
		const res = createMockRes();
		const req = createMockReq("{}", {
			authorization: `Assertion ${assertionToken("human-b", "cred-b", "kill:fleet")}`,
		});

		const handled = await handleKillAuthorizationRoutes(
			"POST",
			"/v1/kill-authorization/requests/req-1/approve",
			req,
			res,
			mockService,
		);
	...
	it("approve on unknown request → 404 (valid token for generic action)", async () => {
		const res = createMockRes();
		const req = createMockReq("{}", {
			authorization: `Assertion ${assertionToken("human-b", "cred-b", "kill:unknown")}`,
		});

		const handled = await handleKillAuthorizationRoutes(
			"POST",
			"/v1/kill-authorization/requests/nope/approve",
			req,
			res,
			mockService,
		);
	...
	it("approve on unknown request with a BAD token → 403 (no request-ID enumeration)", async () => {
		const res = createMockRes();
		const req = createMockReq("{}", {
			authorization: "Assertion not-a-valid-token",
		});
...
describe("POST /v1/kill-authorization/policy-change — quorum-gated flag change", () => {
	it("quorum mode: single signature → 202 PENDING_QUORUM (rejected as single-signature change)", async () => {
		...
		const handled = await handleKillAuthorizationRoutes(
			"POST",
			"/v1/kill-authorization/policy-change",
			req,
			res,
			mockService,
		);
