...
mock.module(dbPath, () => {
	...
	const makeQuery = () => {
		...
		const chain: any = {
			...
			// biome-ignore lint/suspicious/noThenProperty: intentional thenable mock — the route `await`s this Drizzle-style query builder, so it must expose `then`.
			then(resolve: any) {
				...
				if (t === "webhook_api_keys") {
					...
					result =
						eqVal !== null
							? state.audits.filter((a) => a.keyId === eqVal)
							: [...state.audits];
				}
				...
				values: (row: any) => {
					...
					if (t === "webhook_api_keys") {
						if (state.keys.some((k) => k.apiKeyHash === row.apiKeyHash)) {
							throw new Error(
								"UNIQUE constraint failed: webhook_api_keys.api_key_hash",
							);
						}
						state.keys.push({
							...row,
							id: row.id || `wk_test_${state.nextAuditId++}`,
						});
					} else if (t === "webhook_api_key_audit") {
						state.audits.push({
...
describe("handleApiKeysRoutes — admin", () => {
	...
	it("listKeys: returns empty array when no keys", async () => {
		...
		const res = makeRes();
		const handled = await handleApiKeysRoutes(
			"GET",
			"/v1/admin/api-keys",
			req,
			res,
	...
	it("revokeKey: 404 on missing key", async () => {
		const req = makeReq(
			"/v1/admin/api-keys/wk_does_not_exist",
			"DELETE",
			undefined,
			ADMIN_AUTH,
		);
	...
	it("revokeKey: idempotent (204 even if already revoked)", async () => {
		...
		await handleApiKeysRoutes(
			"DELETE",
			`/v1/admin/api-keys/${id}`,
			makeReq(`/v1/admin/api-keys/${id}`, "DELETE", undefined, ADMIN_AUTH),
			makeRes(),
			"127.0.0.1",
		);
		// Revoke again
	...
	it("rotateKey: creates new + revokes old", async () => {
		...
		const createReq = makeReq(
			"/v1/admin/api-keys",
			"POST",
			{ name: "rotate-me", scopes: "live-chat,blog-pipeline" },
			ADMIN_AUTH,
		);
		const createRes = makeRes();
		await handleApiKeysRoutes(
	...
	it("rotateKey: 404 on missing key", async () => {
		const req = makeReq(
			"/v1/admin/api-keys/wk_missing/rotate",
			"POST",
			undefined,
			ADMIN_AUTH,
		);
	...
	it("rotateKey: 409 on already-revoked key", async () => {
		...
		const createReq = makeReq(
			"/v1/admin/api-keys",
			"POST",
			{ name: "already-dead", scopes: "live-chat" },
			ADMIN_AUTH,
		);
		const createRes = makeRes();
		await handleApiKeysRoutes(
		...
		const handled = await handleApiKeysRoutes(
			...
			makeReq(`/v1/admin/api-keys/${id}/rotate`, "POST", undefined, ADMIN_AUTH),
			res,
			"127.0.0.1",
		);
		expect(handled).toBe(true);
...
describe("handleApiKeysRoutes — internal (openclaw-webhook hot path)", () => {
	...
	it("lookup: requires X-Internal-Key", async () => {
		...
		const handled = await handleApiKeysRoutes(
			"GET",
			"/v1/internal/api-keys/lookup?prefix=wk_kvlGr",
			req,
			res,
			"127.0.0.1",
		);
	...
	it("lookup: rejects wrong internal key", async () => {
		const req = {
			...makeReq("/v1/internal/api-keys/lookup?prefix=wk_kvlGr", "GET"),
	...
	it("lookup: requires prefix query param", async () => {
		const req = {
			...makeReq("/v1/internal/api-keys/lookup", "GET"),
			headers: { "x-internal-key": INTERNAL_KEY },
		};
		const res = makeRes();
		const handled = await handleApiKeysRoutes(
			"GET",
			"/v1/internal/api-keys/lookup",
			req,
			res,
			"127.0.0.1",
		);
	...
	it("lookup: returns 404 for unknown prefix", async () => {
		const req = {
			...makeReq("/v1/internal/api-keys/lookup?prefix=wk_unkno", "GET"),
			headers: { "x-internal-key": INTERNAL_KEY },
		};
		const res = makeRes();
		const handled = await handleApiKeysRoutes(
			"GET",
			"/v1/internal/api-keys/lookup?prefix=wk_unkno",
			req,
			res,
			"127.0.0.1",
		);
...
describe("F9: rotateKey TOCTOU race", () => {
	...
	it("concurrent rotation: exactly one succeeds, no double-insert", async () => {
		...
		const createReq = makeReq(
			"/v1/admin/api-keys",
			"POST",
			{ name: "race-key", scopes: "live-chat" },
			ADMIN_AUTH,
		);
		const createRes = makeRes();
		await handleApiKeysRoutes(
		...
		const rotateReq1 = makeReq(
			`/v1/admin/api-keys/${oldId}/rotate`,
			"POST",
			undefined,
			ADMIN_AUTH,
		);
		const rotateReq2 = makeReq(
			`/v1/admin/api-keys/${oldId}/rotate`,
			"POST",
			undefined,
			ADMIN_AUTH,
		...
		const rotateAudits = state.audits.filter(
			(a) => a.action === "rotate" && a.keyId === oldId,
		);
