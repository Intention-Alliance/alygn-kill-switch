...
import {
	handleInternalKillSwitchRoutes,
	resetTransitionRateLimiter,
} from "../internal-kill-switch";
...
describe("handleInternalKillSwitchRoutes — auth", () => {
	...
	it("passes with the correct x-internal-key header", async () => {
		...
		expect(r.body as { ok?: boolean; newState?: string }).toMatchObject({
			ok: true,
			newState: "STOPPED",
		});
	});
...
describe("handleInternalKillSwitchRoutes — transitions", () => {
	it("STOPPED transition works and passes initiatedBy/reason", async () => {
		...
		expect(r.status).toBe(200);
		expect(
			r.body as { ok?: boolean; newState?: string; previousState?: string },
		).toMatchObject({
			ok: true,
			newState: "STOPPED",
	...
	it("threads machineId from the request body into transition metadata (P2-A)", async () => {
		...
		expect(handled).toBe(true);
		expect(r.status).toBe(200);
		expect(r.body as { machineId?: string | null }).toMatchObject({
			machineId: "machine-abc-123",
		});
		expect(service.transitions).toHaveLength(1);
		expect(service.transitions[0].metadata).toMatchObject({
	...
	it("already-STOPPED is idempotent (409, no double-transition)", async () => {
		...
		expect(r.body as { current?: string }).toMatchObject({
			current: "STOPPED",
		});
	});
