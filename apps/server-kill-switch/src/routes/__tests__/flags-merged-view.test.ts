...
mock.module("../../db/index", () => {
	const _lastEq: any = null;
	const _lastAnd: any[] | null = null;
	...
	function storeFor(table: any): any[] {
		...
		if (name.includes("machine_flag") || name === "machineFlags")
			return machineFlagsStore;
...
describe("GET /v1/machines/:id/flags — merged view (F2/F3)", () => {
	it("returns all predefined flags in document order", async () => {
		...
		const handled = await handleFlagsRoutes(
			"GET",
			"/v1/machines/machine-andlersrv/flags",
			req,
			res,
			"admin",
			"admin",
		...
		const autoStop = body.flags.find(
			(f: any) => f.key === "auto_stop_threshold",
		);
	...
	it("surfaces overrides for predefined flags with overridden=true", async () => {
		...
		expect(handled).toBe(true);
		const body = getJson(res);
		const interception = body.flags.find(
			(f: any) => f.key === "llm_interception_enabled",
		);
