...
mock.module("../../db/index", () => {
	...
	function applyFlagFilter(items: any[]): any[] {
		...
		const filtered = items.filter(
			(f: any) => f.id === _lastEqValue || f.flagId === _lastEqValue,
		);
		_lastEqValue = null;
		return filtered;
	}
	...
	function makeSelect() {
		return {
			from(table: any) {
				...
				const isAuditLog =
					tableName.includes("flag_audit_log") || tableName === "flagAuditLog";
							...
							run() {
								...
								if (idx >= 0) {
									...
									if (updates.value !== undefined)
										existing.value = Boolean(updates.value);
...
describe("handleFlagsRoutes", () => {
	...
	describe("PUT /v1/flags/:id — update flag", () => {
		it("PUT /v1/flags/:id update → 200", async () => {
			...
			const res = createMockRes();
			const req = createMockReq(
				JSON.stringify({ key: "new_key", value: true }),
			);
			const handled = await handleFlagsRoutes(
				"PUT",
				"/v1/flags/flag-xyz",
