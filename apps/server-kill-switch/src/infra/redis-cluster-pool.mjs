...
export class RedisPool extends EventEmitter {
	...
	constructor(opts = {}) {
		super();
		this.urls = Array.isArray(opts.urls)
			? opts.urls
			: [opts.urls || "redis://localhost:6379"];
	...
	async connect() {
		...
		console.log(
			`[redis-pool] connecting to ${this.urls.length} URL(s): ${this.urls.join(", ")}`,
		);
	...
	async healthCheck() {
		...
		try {
			console.log(
				`[redis-pool] healthCheck: urls=${this.urls.length}, clientType=${this._cluster.constructor?.name}`,
			);
			...
			console.error(
				`[redis-pool] healthCheck error: ${err.message}, clientType=${this._cluster.constructor?.name}`,
			);
