...
const REQUIRED_ENV = {
	BETTER_AUTH_SECRET: {
		minLength: 32,
		description:
			"Better Auth session signing secret (used to encrypt session cookies)",
	},
	...
	ADMIN_UI_API_KEY: {
		minLength: 32,
		description:
			"Bearer token for /api/admin/secrets/* endpoints (rotate, view, audit)",
	},
	KILL_SWITCH_INTERNAL_KEY: {
		minLength: 32,
		description:
			"Shared secret for /v1/internal/api-keys/* (the openclaw-webhook uses this to look up keys)",
	},
	AUDIT_HMAC_KEY: {
		minLength: 32,
		description:
			"Server-side HMAC key for the immutable audit chain (ADR-140 §6.2 — must NOT live in the SQLite file)",
	},
} as const;
...
export function validateEnvironment(): void {
	...
	const secrets: EnvCheck[] = [
		{
			...
			minLength: 32,
			description: REQUIRED_ENV.BETTER_AUTH_SECRET.description,
		},
		{
			name: "KILL_SWITCH_AUTH_TOKEN",
			minLength: 16,
			description: REQUIRED_ENV.KILL_SWITCH_AUTH_TOKEN.description,
		},
		{
			name: "KILL_SWITCH_API_KEY",
			minLength: 16,
			description: REQUIRED_ENV.KILL_SWITCH_API_KEY.description,
		},
		{
			name: "ADMIN_UI_API_KEY",
			minLength: 32,
			description: REQUIRED_ENV.ADMIN_UI_API_KEY.description,
		},
		{
			name: "KILL_SWITCH_INTERNAL_KEY",
			minLength: 32,
			description: REQUIRED_ENV.KILL_SWITCH_INTERNAL_KEY.description,
		},
		{
			name: "AUDIT_HMAC_KEY",
			minLength: 32,
			description: REQUIRED_ENV.AUDIT_HMAC_KEY.description,
		},
	];
	...
	if (!redisVal) {
		failures.push(
			`Missing environment variable: REDIS_URL — ${REQUIRED_ENV.REDIS_URL.description}`,
		);
	}
...
export function validateVerifierConfig(verification: VerificationConfig): void {
	...
	if (
		!verification.verifierModel ||
		verification.verifierModel.trim().length === 0
	) {
		failures.push(
			"verifierModel must be non-empty when verification is enabled",
		);
	}
	...
	try {
		...
		if (parsedUrl.protocol !== "http:" && parsedUrl.protocol !== "https:") {
			failures.push(
				`verifierBaseUrl must use http(s), got "${parsedUrl.protocol}"`,
			);
		}
	} catch {
		failures.push(
			`verifierBaseUrl is not a valid URL: "${verification.verifierBaseUrl}"`,
		);
	}
