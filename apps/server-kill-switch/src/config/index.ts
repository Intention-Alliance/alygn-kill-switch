...
export function detectEnvironment(): Environment {
	const raw =
		process.env.KILL_SWITCH_ENV || process.env.NODE_ENV || "development";
...
function buildEnvOverrides(): Partial<AppConfig> {
	...
	if (env.KILL_SWITCH_PORT) {
		(overrides as any).server = {
			...((overrides as any).server || {}),
			port: parseInt(env.KILL_SWITCH_PORT, 10),
		};
	}
	...
	if (env.KILL_SWITCH_VERIFICATION_FEATURE_FLAG) {
		(overrides as any).features = {
			...
			killSwitchVerificationEnabled:
				env.KILL_SWITCH_VERIFICATION_FEATURE_FLAG === "true",
		};
	}

	// Telemetry
	if (env.OTEL_EXPORTER_OTLP_ENDPOINT) {
		(overrides as any).telemetry = {
			...((overrides as any).telemetry || {}),
	...
	if (Object.keys(webauthnOverrides).length > 0) {
		(overrides as Record<string, unknown>).webauthn = {
			...((overrides as Record<string, unknown>).webauthn as
	...
	if (env.KILL_SWITCH_VERIFIER_MODEL)
		verificationOverrides.verifierModel = env.KILL_SWITCH_VERIFIER_MODEL;
	if (env.KILL_SWITCH_VERIFIER_BASE_URL)
		verificationOverrides.verifierBaseUrl = env.KILL_SWITCH_VERIFIER_BASE_URL;
	if (env.KILL_SWITCH_VERIFIER_TIMEOUT_MS) {
		verificationOverrides.verifierTimeoutMs = parseInt(
			env.KILL_SWITCH_VERIFIER_TIMEOUT_MS,
			10,
		);
	}
	if (env.KILL_SWITCH_VERIFY_ENABLED) {
		verificationOverrides.verifyEnabled =
			env.KILL_SWITCH_VERIFY_ENABLED === "true";
	}
	if (env.KILL_SWITCH_VERIFY_MODE) {
		verificationOverrides.verifyMode =
			env.KILL_SWITCH_VERIFY_MODE as AppConfig["verification"]["verifyMode"];
	}
	if (env.KILL_SWITCH_VERIFIER_SYSTEM_PROMPT_PATH) {
		verificationOverrides.verifierSystemPromptPath =
			env.KILL_SWITCH_VERIFIER_SYSTEM_PROMPT_PATH;
	}
	if (env.KILL_SWITCH_VERIFIER_TARGET_MODEL) {
		verificationOverrides.verifierTargetModel =
			env.KILL_SWITCH_VERIFIER_TARGET_MODEL;
	}
	if (Object.keys(verificationOverrides).length > 0) {
		(overrides as Record<string, unknown>).verification = {
			...((overrides as Record<string, unknown>).verification as
				| Record<string, unknown>
				| undefined),
