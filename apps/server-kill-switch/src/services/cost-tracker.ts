...
const REDIS_READ_COST_PER_1000 = parseFloat(
	process.env.COST_REDIS_READ_PER_1K || "0.0001",
);
const REDIS_WRITE_COST_PER_1000 = parseFloat(
	process.env.COST_REDIS_WRITE_PER_1K || "0.0002",
);
const REDIS_PUBSUB_COST_PER_1000 = parseFloat(
	process.env.COST_REDIS_PUBSUB_PER_1K || "0.00015",
);
const API_REQUEST_COST_PER_1000 = parseFloat(
	process.env.COST_API_REQUEST_PER_1K || "0.00005",
);
const COMPUTE_COST_PER_HOUR = parseFloat(
	process.env.COST_COMPUTE_PER_HOUR || "0.032",
);
...
class CostTracker {
	...
	getReport(): CostReport {
		...
		const totalApiRequests = Object.values(this.apiRequests).reduce(
			(a, b) => a + b,
			0,
		);
	...
	shouldThrottle(endpoint: string): boolean {
		...
		const hoursElapsed = Math.max(
			(Date.now() - this.startTime) / 3600000,
			0.001,
		);
