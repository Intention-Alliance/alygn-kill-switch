...
export class WebSocketManager {
	...
	setRedisSubscribe(fn: any): void {
		for (const channel of REDIS_CHANNELS) {
			fn(channel, (message: string) => {
				this.onRedisMessage(channel, message);
			});
		}
		console.log(
			"[ws] Subscribed to Redis channels:",
			REDIS_CHANNELS.join(", "),
		);
	}
	...
	handleBunUpgrade(ws: any): void {
		...
		client.heartbeatTimer = setInterval(() => {
			if (!client.alive) {
				...
				this.removeClient(client, 4005);
				return;
			}
			if (Date.now() - client.lastPong > 40000) {
				clearInterval(client.heartbeatTimer!);
				this.removeClient(client, 4005);
				return;
			}
	...
	private onRedisMessage(channel: string, message: string): void {
		try {
			...
			switch (channel) {
				case "bcp:kill-switch:chaos":
					wsMessage = { type: "state-change", payload: parsed };
					break;
				case "bcp:flags:updates":
					wsMessage = { type: "flag-update", payload: parsed };
					break;
				case "bcp:agents:events":
					wsMessage = { type: "agent-event", payload: parsed };
					break;
				case "bcp:machines:events":
					wsMessage = parsed.type
						? parsed
						: { type: "machine-event", payload: parsed };
					break;
				case "bcp:settings:updates":
					wsMessage = { type: "settings-update", payload: parsed };
					break;
				case "bcp:machines:metrics":
					wsMessage = parsed.type
						? parsed
						: { type: "machine-metrics", payload: parsed };
					break;
				case "bcp:verification:events":
					...
					wsMessage = parsed.type
						? parsed
						: { type: "verification-event", payload: parsed };
					break;
			}
	...
	broadcastFlagUpdate(payload: any): void {
		this.broadcast({ type: "flag-update", payload });
	}
	broadcastAgentEvent(payload: any): void {
		this.broadcast({ type: "agent-event", payload });
	}
	broadcastMachineEvent(type: string, payload: Record<string, unknown>): void {
