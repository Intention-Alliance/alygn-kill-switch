...
export interface VerifierOpts {
	...
	fetchImpl?: (
		...
		init?: RequestInit,
	) => Promise<Response>; // injectable for tests
}
...
function loadSystemPrompt(examples?: ICLExample[]): string {
	...
	if (process.env.KILL_SWITCH_VERIFIER_SYSTEM_PROMPT_PATH) {
		...
		try {
			...
			if (code === "ENOENT") {
				console.debug(
					"[verifier] Custom system prompt file not found, using dynamic builder:",
					path,
				);
			} else {
				console.warn(
					"[verifier] Failed to load custom system prompt, using dynamic builder:",
	...
	const effectiveExamples = examples ?? [
		...CANONICAL_EXAMPLES,
		...INJECTION_EXAMPLES,
	];
 ...
 */
export function extractVerdict(rawText: string): {
	verdict: Verdict;
	degraded: boolean;
} {
...
export class InferenceVerifier {
	...
	constructor(opts: VerifierOpts = {}) {
		this.model =
			opts.model ?? process.env.KILL_SWITCH_VERIFIER_MODEL ?? DEFAULT_MODEL;
		this.baseUrl = (
			opts.baseUrl ??
			process.env.KILL_SWITCH_VERIFIER_BASE_URL ??
			DEFAULT_BASE_URL
		).replace(/\/+$/, "");
		this.timeoutMs =
			opts.timeoutMs ??
			Number(process.env.KILL_SWITCH_VERIFIER_TIMEOUT_MS ?? DEFAULT_TIMEOUT_MS);
	...
	async verify(input: {
		...
		try {
			...
			console.error(
				"[verifier] Model call failed (degraded):",
				err instanceof Error ? err.message : err,
			);
			return {
				verdict: "REVIEW",
		...
		if (parseDegraded) {
			console.warn(
				"[verifier] No verdict token in model response — defaulting to REVIEW (degraded)",
			);
		}
