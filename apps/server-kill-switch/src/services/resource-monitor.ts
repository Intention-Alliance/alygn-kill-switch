...
const CPU_WARNING_THRESHOLD = parseFloat(
	process.env.RESOURCE_CPU_WARNING || "70",
);
const CPU_CRITICAL_THRESHOLD = parseFloat(
	process.env.RESOURCE_CPU_CRITICAL || "90",
);
const MEMORY_WARNING_THRESHOLD = parseFloat(
	process.env.RESOURCE_MEMORY_WARNING || "80",
);
const MEMORY_CRITICAL_THRESHOLD = parseFloat(
	process.env.RESOURCE_MEMORY_CRITICAL || "90",
);
const DISK_WARNING_THRESHOLD = parseFloat(
	process.env.RESOURCE_DISK_WARNING || "85",
);
const DISK_CRITICAL_THRESHOLD = parseFloat(
	process.env.RESOURCE_DISK_CRITICAL || "95",
);
...
export function getResourceStats(): ResourceStats {
	...
	const cpuPercent =
		elapsedMicros > 0
			? Math.round((totalCpuMicros / elapsedMicros) * 10000) / 100
			: 0;
	...
	try {
		...
		const availableBlocks =
			(fsStats as any).bavail || (fsStats as any).blocks || 0;
