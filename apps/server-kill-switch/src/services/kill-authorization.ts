...
export type KillAuthorizationStatus =
	| "PENDING_QUORUM"
	| "EXECUTED"
	| "EXPIRED"
	| "REJECTED";
...
function toRequest(
	row: typeof killAuthorizationRequests.$inferSelect,
): KillAuthorizationRequest {
	return {
		id: row.id,
...
async function loadRequest(
	requestId: string,
	dbLike: DbLike = db,
): Promise<KillAuthorizationRequest | null> {
...
async function createRequest(params: {
	...
	await db
		.insert(killAuthorizationRequests)
		.values({
			id: request.id,
...
export async function approve(
	...
	if (alreadySigned) {
		throw new KillAuthorizationError(
			"This credential already signed the request",
			"ALREADY_SIGNED",
		);
	}
