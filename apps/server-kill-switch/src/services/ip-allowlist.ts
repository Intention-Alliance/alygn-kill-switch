...
export async function refreshTailscaleDns(): Promise<string[]> {
	...
	if (!SECURE_NET_HOSTNAME) {
		if (!warnedNoHostname) {
			...
			console.warn(
				"[ip-allowlist] SECURE_NET_HOSTNAME not set — Tailscale DNS allowlist refresh disabled",
			);
		}
	...
	try {
		...
		console.log(
			`[ip-allowlist] DNS resolved ${SECURE_NET_HOSTNAME} → [${addresses.join(", ")}]`,
		);
		return addresses;
	} catch (err: any) {
		console.error(
			`[ip-allowlist] DNS resolution failed for ${SECURE_NET_HOSTNAME}: ${err.message}`,
		);
...
const CIDR_RANGES = parseEnvList(
