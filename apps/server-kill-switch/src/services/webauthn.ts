...
function takeChallenge(
	...
	if (Date.now() - entry.createdAt > getConfig().webauthn.challengeTtlMs)
		return null;
...
function assertionTokenSecret(): string {
	...
	if (!secret) {
		throw new Error(
			"Missing WEBAUTHN_ASSERTION_TOKEN_SECRET (or BETTER_AUTH_SECRET)",
		);
	}
	return secret;
}
...
function signAssertionToken(payload: AssertionTokenPayload): string {
	...
	const sig = createHmac("sha256", assertionTokenSecret())
		.update(body)
		.digest("base64url");
	return `${body}.${sig}`;
}
...
function verifyAssertionToken(token: string): AssertionTokenPayload | null {
	...
	const expected = createHmac("sha256", assertionTokenSecret())
		.update(body)
		.digest("base64url");
...
export async function listActiveCredentialsForUser(
	userId: string,
...
export async function finishRegistration({
	...
	if (pending.userId !== userId) {
		throw new WebAuthnError(
			"Registration challenge was issued for a different user",
			"CHALLENGE_MISMATCH",
		);
	}
...
export async function startAssertion({
	...
	if (userId) {
		...
		if (allowCredentials.length === 0) {
			throw new WebAuthnError(
				"No registered authenticators for this user",
				"NO_CREDENTIALS",
			);
		}
	}
...
export async function finishAssertion({
	...
	if (!pending) {
		throw new WebAuthnError(
			"Assertion challenge missing, expired, or already used",
			"CHALLENGE_INVALID",
		);
	}
	if (!pending.action) {
		throw new WebAuthnError(
			"Assertion challenge missing bound action",
			"CHALLENGE_INVALID",
		);
	}

	const authResponse = response as unknown as Parameters<
	...
	if (pending.userId && credential.userId !== pending.userId) {
		throw new WebAuthnError(
			"Credential does not belong to the challenge user",
			"CREDENTIAL_MISMATCH",
		);
	}
	...
	if (newCounter <= credential.counter) {
		throw new WebAuthnError(
			"Credential counter did not advance — possible replay",
			"REPLAY_DETECTED",
		);
	}
	...
	const assertionToken = mintAssertionToken(
		credential.userId,
...
export async function finishLoginAssertion({
	...
	if (!pending) {
		throw new WebAuthnError(
			"Login challenge missing, expired, or already used",
			"CHALLENGE_INVALID",
		);
	}
	...
	if (!verification.verified) {
		throw new WebAuthnError(
			"Login assertion verification failed",
			"VERIFICATION_FAILED",
		);
	}
	...
	if (!user) {
		throw new WebAuthnError(
			"Credential owner no longer exists",
			"USER_NOT_FOUND",
		);
	}
...
export function verifyAssertionTokenForAction({
	...
	if (!payload) {
		throw new WebAuthnError(
			"Invalid or expired assertion token",
			"TOKEN_INVALID",
		);
	}
	if (payload.action !== action) {
		throw new WebAuthnError(
			"Assertion token is bound to a different action",
			"TOKEN_ACTION_MISMATCH",
		);
	}
	return {
		userId: payload.sub,
		credentialId: payload.cred,
