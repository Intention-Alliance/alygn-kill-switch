...
function publishEvent(payload: unknown): void {
	if (!_publish) return;
	void _publish(DISCOVERY_EVENTS_CHANNEL, JSON.stringify(payload)).catch(
		(err) => {
			console.warn(
				"[registry-scheduler] Redis publish dropped:",
...
async function runProviderProbeTick(): Promise<void> {
	...
	for (const machine of admitted) {
		try {
			...
			console.warn(
				"[registry-scheduler] Provider probe failed for machine",
				machine.id,
				":",
				err instanceof Error ? err.message : err,
			);
...
async function runReconciliationTick(): Promise<void> {
	...
	const stale = await db
		.select({
			...
			lastSeen: discoveredMachines.lastSeen,
		})
		.from(discoveredMachines)
				...
				offline: stale.map((m) => ({
					...
					lastSeen:
						m.lastSeen instanceof Date
							...
							: new Date(Number(m.lastSeen)).toISOString(),
				})),
				timestamp: new Date().toISOString(),
			},
		});
...
export function startRegistryScheduler(opts: RegistrySchedulerOpts = {}): void {
	...
	console.log(
		`[registry-scheduler] Starting: sweep every ${sweepIntervalMs}ms, probe every ${probeIntervalMs}ms, heartbeat timeout ${_heartbeatTimeoutMs}ms`,
	);

	// Startup seed sweep — populate the registry within seconds, not a minute.
	void runSweepTick().catch((err) => {
		console.error(
			"[registry-scheduler] Startup sweep failed:",
			err instanceof Error ? err.message : err,
		);
	});
	...
	_sweepInterval = setInterval(async () => {
		...
		try {
			...
			console.error(
				"[registry-scheduler] Sweep tick error:",
				err instanceof Error ? err.message : err,
			);
		} finally {
			_running = false;
	...
	_probeInterval = setInterval(async () => {
		...
		try {
			...
			console.error(
				"[registry-scheduler] Probe tick error:",
				err instanceof Error ? err.message : err,
			);
		} finally {
			_running = false;
