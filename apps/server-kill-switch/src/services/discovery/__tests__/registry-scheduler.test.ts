		...
		select: () => ({
			from: (_table: unknown) => ({
				where: (cond: any) => {
					...
					if (cond?.__lt !== undefined)
						_ltValues.push(cond?.__leftName, cond?.__lt);
					return {
...
mock.module("../../../db/index", () => {
	const dbMock = {
		select: () => ({
			from: (table: any) => {
				...
				if (name === "discovered_machine")
					return makeTableProxy(machinesStore).select().from(table);
				if (name === "discovered_provider")
					return makeTableProxy(providersStore).select().from(table);
		...
		insert: (table: any) => {
			const name = tableName(table);
			if (name === "discovered_machine")
				return makeTableProxy(machinesStore).insert();
			if (name === "discovered_provider")
				return makeTableProxy(providersStore).insert();
			return makeTableProxy([]).insert();
		},
		update: (table: any) => {
			const name = tableName(table);
			if (name === "discovered_machine")
				return makeTableProxy(machinesStore).update();
			if (name === "discovered_provider")
				return makeTableProxy(providersStore).update();
