	...
	const dbMock = {
		select: () => ({
			from: (table: any) => {
				...
				if (name === "agent")
					return makeTableProxy(agentsStore).select().from(table);
...
describe("HeartbeatCollector", () => {
	it("delegates heartbeat to the orchestrator and returns drift + signature", async () => {
		const orch = makeOrchestrator({
			drift: { event: "tamper", severity: "high" },
		});
		...
		expect(orch.heartbeatCalls[0]).toEqual({
			machineId: "m-1",
			hostname: "worker-01",
		});
		expect(result.drift as unknown).toEqual({
			event: "tamper",
			severity: "high",
		});
		expect(result.signature as unknown).toEqual({
			alg: "sha256",
			value: "sig",
		});
