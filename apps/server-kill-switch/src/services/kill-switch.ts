...
export class KillSwitchService {
	...
	private _stateChangeListeners: Array<
		(entry: AuditEntry) => void | Promise<void>
	> = [];
			...
			async (span: any) => {
				...
				if (this.auditLog.length > KillSwitchService.HOT_CACHE_SIZE) {
					this.auditLog = this.auditLog.slice(
						-KillSwitchService.HOT_CACHE_SIZE,
					);
				}
				...
				try {
					...
					console.error(
						"[kill-switch] Failed to persist audit entry to DB:",
						err,
					);
				}
	...
	async loadAuditFromDb(): Promise<void> {
		...
		try {
			...
			console.log(
				`[kill-switch] Loaded ${this.auditLog.length} audit entries from DB`,
			);
		} catch (err) {
	...
	async seedInitialAuditEntry(): Promise<void> {
		try {
			...
			await appendAuditEntry({
				...
				plainExplanation:
					"System initialized — kill switch running (seed entry, chain genesis).",
			});
			...
			console.error(
				"[kill-switch] Failed to seed initial audit entry:",
