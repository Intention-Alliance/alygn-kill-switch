...
const blockedIps = new Map<
	string,
	{ blockedAt: number; reason: string; unblockAt: number }
>();
const AUTH_FAILURE_THRESHOLD = parseInt(
	process.env.INCIDENT_AUTH_FAILURE_THRESHOLD || "10",
	10,
);
const AUTH_FAILURE_WINDOW_MS = parseInt(
	process.env.INCIDENT_AUTH_FAILURE_WINDOW_MS || "300000",
	10,
); // 5 min
const IP_BLOCK_DURATION_MS = parseInt(
	process.env.INCIDENT_IP_BLOCK_DURATION_MS || "3600000",
	10,
); // 1 hour
const ERROR_RATE_THRESHOLD = parseFloat(
	process.env.INCIDENT_ERROR_RATE_THRESHOLD || "0.5",
); // 50%
...
class IncidentResponseService {
	...
	async detectRedisFailure(
		...
		try {
			...
			if (this.circuitBreakerOpen && health.redis.redis === "OK") {
				...
				const lastIncident = this.incidents.find(
					(i) => i.type === "redis_failure" && i.status !== "resolved",
				);
				if (lastIncident) {
	...
	detectHighLoad(): Incident | null {
		...
		const hasHighCpu = alerts.some(
			(a) => a.type === "cpu" && a.level === "critical",
		);
	...
	private createIncident(
		...
		console.error(
			`[incident-response] ${severity.toUpperCase()}: ${message} → ${autoResponse}`,
		);
