...
import type { CollectorState, HardwareMetric } from "../types";
	...
	public startCollecting(): void {
		...
		this.collectionTimer = setInterval(() => {
			this.collectAndProcess(false).catch((error: unknown) => {
				console.error(
					"[metric-collector] Standard collection error:",
		...
		this.criticalCollectionTimer = setInterval(() => {
			this.collectAndProcess(true).catch((error: unknown) => {
				console.error(
					"[metric-collector] Critical collection error:",
					(error as Error).message,
				);
			});
		...
		this.collectAndProcess(false).catch((error: unknown) => {
			console.error(
				"[metric-collector] Initial collection error:",
				(error as Error).message,
			);
		});
	}
		...
		try {
			for (const monitor of this.hardwareMonitors) {
				...
				for (const metric of rawMetrics) {
					...
					const previousValue =
						this.collectorState.lastMetricValues.get(deduplicationKey);

					if (
						previousValue !== undefined &&
						previousValue === metric.metricValue
					) {
						continue; // Skip — value hasn't changed
					}

					// Update tracked value
					this.collectorState.lastMetricValues.set(
						deduplicationKey,
						metric.metricValue,
					);
