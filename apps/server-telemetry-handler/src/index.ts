...
export async function startTelemetryServer(
		...
		if (url === "/health" && method === "GET") {
			...
			const healthPayload = {
				...
				collector: {
					...
					lastFlushTimestamp: new Date(
						collectorState.lastFlushTimestamp,
					).toISOString(),
				},
				timestamp: new Date().toISOString(),
			};
	...
	server.listen(port, () => {
		...
		console.log(
			`   DB: SQLite at ${config.dataDir}/telemetry.sqlite (WAL mode)`,
		);
		console.log(
			`   Collection interval: ${config.collectionIntervalSeconds}s (critical: ${config.criticalCollectionIntervalSeconds}s)`,
		);
		console.log(
			`   Batch flush: ${config.batchFlushIntervalSeconds}s / ${config.batchMaxSize} max`,
		);
		console.log(
			`   Kill switch trigger: ${config.enableKillSwitchTrigger ? "enabled" : "disabled"}`,
		);
		...
		console.log(
			`   Monitors: ${service.monitors.map((m) => m.name).join(", ")}`,
		);
	});
...
if (
	...
	import.meta.path?.endsWith("index.js")
) {
