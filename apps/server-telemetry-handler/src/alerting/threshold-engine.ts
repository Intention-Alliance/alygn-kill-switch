...
import type { KillSwitchBridge } from "./kill-switch-bridge";
...
export class ThresholdEngine {
	...
	public async evaluate(
		...
		const matchingRules = this.thresholdRules.filter(
			(rule) =>
				rule.monitorName === metric.monitorName &&
				rule.metricName === metric.metricName,
		);
		...
		const severityPriority: SeverityLevel[] = [
			"emergency",
			"critical",
			"warning",
		];
		...
		for (const rule of matchingRules) {
			...
			if (isExceeded) {
				if (
					worstSeverity === null ||
					severityPriority.indexOf(rule.severity) <
						severityPriority.indexOf(worstSeverity)
				) {
