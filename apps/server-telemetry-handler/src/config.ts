...
export function loadTelemetryConfig(
	...
	const redisUrlsRaw =
		env.REDIS_URLS || env.REDIS_URL || "redis://localhost:6379";
	const redisUrls = redisUrlsRaw
		.split(",")
		.map((url) => url.trim())
		.filter(Boolean);
	...
	return {
		...
		batchMaxSize: parseInt(env.TELEMETRY_BATCH_MAX_SIZE || "100", 10),
		thresholds: {
			gpu: {
				tempCriticalCelsius: parseFloat(env.GPU_TEMP_CRITICAL_CELSIUS || "85"),
				tempEmergencyCelsius: parseFloat(
					env.GPU_TEMP_EMERGENCY_CELSIUS || "95",
				),
			},
			cpu: {
				tempCriticalCelsius: parseFloat(env.CPU_TEMP_CRITICAL_CELSIUS || "80"),
			},
			memory: {
		...
		enableKillSwitchTrigger:
			env.TELEMETRY_ENABLE_KILL_SWITCH_TRIGGER !== "false",
		logLevel: (env.TELEMETRY_LOG_LEVEL ||
			"info") as TelemetryConfig["logLevel"],
