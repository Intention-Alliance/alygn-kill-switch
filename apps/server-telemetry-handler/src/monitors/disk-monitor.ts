...
export class DiskMonitor implements HardwareMonitor {
	...
	public async collect(): Promise<HardwareMetric[]> {
		...
		for (const mountPoint of mountPoints) {
			try {
				...
				const blockSize =
					(fsStats as unknown as Record<string, number>).bsize || 4096;
				const totalBlocks =
					(fsStats as unknown as Record<string, number>).blocks || 0;
				const availableBlocks =
					(fsStats as unknown as Record<string, number>).bavail ||
	...
	private readDiskIOStats(): HardwareMetric[] {
		...
		try {
			...
			console.warn(
				"[disk-monitor] Failed to read /proc/diskstats:",
				(error as Error).message,
			);
		}
