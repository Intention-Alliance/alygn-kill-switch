...
export const telemetryMetrics = sqliteTable(
	...
	(table) => ({
		machineTimeIdx: index("tm_machine_time_idx").on(
			table.machineId,
			table.timestamp,
		),
		monitorTimeIdx: index("tm_monitor_time_idx").on(
			table.monitorName,
			table.timestamp,
		),
		metricTimeIdx: index("tm_metric_time_idx").on(
			table.metricName,
			table.timestamp,
		),
	}),
...
export const telemetryAlerts = sqliteTable(
	"telemetry_alert",
	{
		...
		triggeredKillSwitch: integer("triggered_kill_switch", {
			mode: "boolean",
		}).default(false),
		timestamp: integer("timestamp", { mode: "timestamp" })
			.notNull()
	...
	(table) => ({
		severityTimeIdx: index("ta_severity_time_idx").on(
			table.severity,
			table.timestamp,
		),
		machineTimeIdx: index("ta_machine_time_idx").on(
			table.machineId,
			table.timestamp,
		),
		monitorTimeIdx: index("ta_monitor_time_idx").on(
			table.monitorName,
			table.timestamp,
		),
	}),
