# VC Outreach Campaign Workflow - Afternoon (Recovery)
...
- Wave files stored in `$HOME/.openclaw/workspace/reports/alygn/waves/VC/YYYY-MM-DD.json`
...
- For each entity with status `drafted`:
  - Execute **Phase 6 (Send)** via the `alygn-vc-outreach` skill
  - Send the approved email to the VC contact
...
```txt
📬 [alygn-vc-afternoon] — Today's VC Wave Status
• Sent: X
• Pending: Y
• Failed (retrying): Z
• Exhausted: W
...
- Log all sends to `$HOME/.openclaw/workspace/reports/alygn/waves/VC/logs/sent-email-tracker.json`
- Include: entity id, VC firm name, send timestamp, status

## Phases

- `phase6-send` — Send the approved email to the VC contact
- `phase4-personalize` — Re-personalize for retry attempts

## Expected Outcomes

- All `drafted` entities from today's wave have been sent
...
- Post to Discord: `✅ [alygn-vc-afternoon] Complete — X emails sent, Y retried`
...
- Only send emails that have explicit approval (either via Discord reaction/thread or from a config flag)
- Approved sends are determined by: entity status `drafted` AND no rejection flag
- If no entities are ready to send, post `ℹ️ [alygn-vc-afternoon] No sends due today` and exit cleanly
