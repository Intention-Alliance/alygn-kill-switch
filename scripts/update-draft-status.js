import { getClient, queryDatabase, updatePage, retrievePage } from './shared/notion-client.js';
...
async function findAndUpdateVC(vcId, vcName) {
  ...
  try {
    ...
    if (response.results.length === 0) {
      ...
      // Try by name
      const nameResponse = await queryDatabase(notion, DATABASE_ID, {
        filter: {
          property: 'Name',
          title: {
            contains: vcName.replace('Capital', '').trim()
          }
        },
        page_size: 10
      });
