...
const PENDING_REVIEW_STATE_FILE = "/tmp/gh-pending-review-actions.json";
...
// Reviewer feedback detection (Card 6cd8925f): surface reviewer-body feedback
// (Chanshuk Stage 1, Nikaya Stage 2, CodeRabbit summaries, inline review threads)
// in the parked DM / heartbeat log — not just AndlerRL @wobblus tags.
...
function main(): number {
  ...
  if (newActions.length === 0) {
    ...
    state.lastRunIso = ts();
    saveState(state);
    // Reviewer-feedback scan runs even when no AndlerRL tags exist (Card 6cd8925f).
    const reviewState = loadPendingReviewState();
    const reviewSince = reviewState.lastRunIso;
    const allReviewItems: PendingReviewItem[] = [];
    for (const repo of REPOS) {
      try {
        allReviewItems.push(...fetchPRReviewFeedback(repo, reviewSince));
      } catch (err: any) {
        log(`review scan ${repo} failed: ${err.message ?? err}`);
      }
    }
    const freshItems = dedupePendingItems(allReviewItems, reviewState);
    if (freshItems.length > 0) {
      reviewState.items.push(...freshItems);
      if (reviewState.items.length > 500) reviewState.items = reviewState.items.slice(-500);
      reviewState.lastRunIso = ts();
      savePendingReviewState(reviewState);
      logWork(
        `📋 gh-reply-queue: ${freshItems.length} pending reviewer action item(s) surfaced — ` +
          freshItems.map((i) => `${i.repo}#${i.pr} (${i.author}: ${i.actionPatterns.join(",")})`).join("; "),
      );
      logWork(`  State: ${PENDING_REVIEW_STATE_FILE} — pending-action-items: ${reviewState.items.length}`);
    } else {
      reviewState.lastRunIso = ts();
      savePendingReviewState(reviewState);
      log("no new reviewer feedback items");
    }
    return 0;
  }
...
// ============================================================
// REVIEWER FEEDBACK DETECTION (Card 6cd8925f)
// ============================================================
...
function loadPendingReviewState(): PendingReviewState {
  if (existsSync(PENDING_REVIEW_STATE_FILE)) {
    try {
      return JSON.parse(readFileSync(PENDING_REVIEW_STATE_FILE, "utf-8"));
    } catch {
      log(`could not parse ${PENDING_REVIEW_STATE_FILE}, starting fresh`);
    }
  }
  return { lastRunIso: "1970-01-01T00:00:00Z", items: [] };
}

function savePendingReviewState(state: PendingReviewState): void {
  writeFileSync(PENDING_REVIEW_STATE_FILE, JSON.stringify(state, null, 2));
}

function isExcludedBot(body: string, author: string): boolean {
  const lower = body.toLowerCase();
  if (EXCLUDED_BOT_PATTERNS.some((p) => p.test(lower))) return true;
  if (author === WOBBLUS_LOGIN) return true;
  return false;
}
...
function fetchPRReviewFeedback(repo: string, sinceIso: string): PendingReviewItem[] {
  // Scan open PRs for reviewer-body feedback: PR review bodies (Stage 1/2 verdicts,
  // CodeRabbit summaries) + inline review threads. Excludes Vercel/bot noise and
  // Wobblus's own comments. Idempotent via PENDING_REVIEW_STATE_FILE dedup.
  ...
  for (const pr of prs) {
    try {
      // PR review bodies (top-level reviews: Stage 1/2 verdicts, CodeRabbit summaries)
      const reviews = JSON.parse(
        gh(`api "repos/${repo}/pulls/${pr.number}/reviews?per_page=50"`),
      ) as { id: number; user: { login: string }; body: string; submitted_at: string }[];
      for (const r of reviews) {
        if (!r.body || r.submitted_at < sinceIso) continue;
        if (isExcludedBot(r.body, r.user?.login ?? "")) continue;
        if (!isReviewerOrBot(r.user?.login ?? "", r.body)) continue;
        const patterns = extractActionPatterns(r.body);
        if (patterns.length === 0 && !ACTION_PATTERN.test(r.body)) continue;
        items.push({
          repo,
          pr: pr.number,
          commentId: r.id,
          ...
          actionPatterns: patterns.length > 0 ? patterns : ["REVIEW"],
          createdAt: r.submitted_at,
          actionedBy: null,
        });
      }
      // Inline review threads (PR review comments on diff lines)
      const inline = JSON.parse(
        gh(`api "repos/${repo}/pulls/${pr.number}/comments?per_page=50"`),
      ) as { id: number; user: { login: string }; body: string; created_at: string }[];
      for (const c of inline) {
        if (!c.body || c.created_at < sinceIso) continue;
        if (isExcludedBot(c.body, c.user?.login ?? "")) continue;
        if (!isReviewerOrBot(c.user?.login ?? "", c.body)) continue;
        const patterns = extractActionPatterns(c.body);
        if (patterns.length === 0 && !ACTION_PATTERN.test(c.body)) continue;
        items.push({
          repo,
          pr: pr.number,
          commentId: c.id,
      ...
      log(`review feedback scan ${repo}#${pr.number} failed: ${err.message ?? err}`);
    }
  }
  return items;
}

function dedupePendingItems(items: PendingReviewItem[], state: PendingReviewState): PendingReviewItem[] {
  const seen = new Set(state.items.map((i) => `${i.repo}#${i.pr}:${i.commentId}`));
  return items.filter((i) => !seen.has(`${i.repo}#${i.pr}:${i.commentId}`));
}
