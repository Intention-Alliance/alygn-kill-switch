...
// Supabase removed — VC Outreach is Notion-only pipeline
...
// logOutreachEmail removed — tracking is Notion-only
  ...
  for (const vc of vcs) {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Tracking is Notion-only — status already updated above
        console.log(`      ✅ Logged to Notion (status: Sent)`);
      } else {
