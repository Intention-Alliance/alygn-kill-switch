...
__export(exports_outreach_email_template, {
  ...
  generateEmailForVC: () => generateEmailHTML,
  generateEmailForMunicipality: () => generateEmail,
  generateEmail: () => generateEmail,
  default: () => outreach_email_template_default
});
function generateEmail(params) {
  ...
  const templates = {
    governance: {
      es: {
        intro: `Alygn es una instituci\xF3n independiente de gobernanza de IA enfocada en hacer que la rendici\xF3n de cuentas, la supervisi\xF3n y la coordinaci\xF3n sean viables para sistemas de IA avanzados que operan a escala global.<br><br>
A medida que los sistemas de IA superan a los actores individuales, la gobernanza no puede ser a\xF1adida retroactivamente. Existimos para apoyar la coordinaci\xF3n entre desarrolladores, operadores e instituciones p\xFAblicas\u2014sin centralizar el control ni afirmar autoridad.`,
    ...
    institutional: {
      es: {
        intro: `Cuando los sistemas de IA escalan m\xE1s all\xE1 del control individual, la coordinaci\xF3n se convierte en el cuello de botella. La supervisi\xF3n tradicional se desmorona cuando ning\xFAn actor individual puede intervenir de manera cre\xEDble por s\xED solo.<br><br>
Alygn es una instituci\xF3n independiente enfocada en hacer que la rendici\xF3n de cuentas, la respuesta de emergencia y la coordinaci\xF3n entre organizaciones funcionen realmente\u2014antes de que las condiciones de crisis impulsen resultados fragmentados.`,
        closing: "Esperando explorar esto con usted.",
    ...
    traiga: {
      es: {
        intro: `El TRAIGA Act (Tecnolog\xEDas de Riesgo de Inteligencia Artificial de Gran Alcance) establece un marco regulatorio para sistemas de IA de alto riesgo.<br><br>
Como instituci\xF3n independiente de gobernanza de IA, Alygn puede apoyar a los municipios en la implementaci\xF3n pr\xE1ctica de estas salvaguardas\u2014sin centralizar el control ni afirmar autoridad.`,
        closing: "Esperando explorar esto con usted.",
...
function buildFooter(variant) {
  ...
  <p style="margin: 0 0 12px 0;">
    ...
    Texas, EE.UU. | ${year} \xA9 All rights reserved.
  </p>
...
class SentEmailTracker {
  ...
  wasAlreadySent(email, partnerName, type) {
    ...
    if (type === "vc" && partnerName) {
      return this.sentEmails[key].some((entry) => entry.email.toLowerCase() === email.toLowerCase() && entry.partnerName === partnerName);
    }
    return this.sentEmails[key].some((entry) => entry.email.toLowerCase() === email.toLowerCase());
  }
  recordSent(params) {
    ...
    let existingIndex = -1;
    if (type === "vc" && partnerName) {
      existingIndex = this.sentEmails[key].findIndex((entry2) => entry2.email.toLowerCase() === email.toLowerCase() && entry2.partnerName === partnerName);
    } else {
      existingIndex = this.sentEmails[key].findIndex((entry2) => entry2.email.toLowerCase() === email.toLowerCase());
    }
    const entry = {
      email,
      name,
  ...
  getSentEntry(email, partnerName, type) {
    ...
    if (!email)
      return null;
    if (type === "vc" && partnerName) {
      return this.sentEmails[key].find((entry) => entry.email.toLowerCase() === email.toLowerCase() && entry.partnerName === partnerName) || null;
    }
    return this.sentEmails[key].find((entry) => entry.email.toLowerCase() === email.toLowerCase()) || null;
  }
  wasPartnerEmailed(email, partnerName) {
    if (!email || !partnerName)
      return false;
    return this.sentEmails.vcs.some((entry) => entry.email.toLowerCase() === email.toLowerCase() && entry.partnerName === partnerName);
  }
...
var exports_EmailValidatorFactory = {};
__export(exports_EmailValidatorFactory, {
  default: () => EmailValidatorFactory_default,
...
var EmailValidatorFactory_default;
var init_EmailValidatorFactory = __esm(() => {
  init_RegexMXValidator();
  init_ZeroBounceValidator();
  EmailValidatorFactory_default = EmailValidatorFactory;
});
...
var COSTA_RICA_CANTONES = [
  ...
  { name: "Desamparados", province: "San Jos\xE9", population: 208741, budget: 80000000 },
  { name: "Puriscal", province: "San Jos\xE9", population: 33484, budget: 25000000 },
  { name: "Tarraz\xFA", province: "San Jos\xE9", population: 16260, budget: 18000000 },
  { name: "Aserr\xED", province: "San Jos\xE9", population: 58912, budget: 35000000 },
  { name: "Mora", province: "San Jos\xE9", population: 28378, budget: 22000000 },
  { name: "Goicoechea", province: "San Jos\xE9", population: 133022, budget: 60000000 },
  { name: "Santa Ana", province: "San Jos\xE9", population: 67496, budget: 40000000 },
  { name: "Alajuelita", province: "San Jos\xE9", population: 89148, budget: 38000000 },
  { name: "V\xE1zquez de Coronado", province: "San Jos\xE9", population: 60882, budget: 32000000 },
  { name: "Acosta", province: "San Jos\xE9", population: 19734, budget: 15000000 },
  { name: "Tib\xE1s", province: "San Jos\xE9", population: 82409, budget: 42000000 },
  { name: "Moravia", province: "San Jos\xE9", population: 57182, budget: 30000000 },
  { name: "Montes de Oca", province: "San Jos\xE9", population: 56935, budget: 35000000 },
  { name: "Turrubares", province: "San Jos\xE9", population: 6964, budget: 8000000 },
  { name: "Dota", province: "San Jos\xE9", population: 9924, budget: 1e7 },
  { name: "Curridabat", province: "San Jos\xE9", population: 77232, budget: 45000000 },
  { name: "P\xE9rez Zeled\xF3n", province: "San Jos\xE9", population: 134534, budget: 65000000 },
  { name: "Le\xF3n Cort\xE9s", province: "San Jos\xE9", population: 11379, budget: 12000000 },
  { name: "Alajuela", province: "Alajuela", population: 254212, budget: 120000000 },
  { name: "San Ram\xF3n", province: "Alajuela", population: 82985, budget: 45000000 },
  ...
  { name: "Po\xE1s", province: "Alajuela", population: 29703, budget: 22000000 },
  ...
  { name: "Sarch\xED", province: "Alajuela", population: 18205, budget: 16000000 },
  ...
  { name: "R\xEDo Cuarto", province: "Alajuela", population: 11980, budget: 12000000 },
  { name: "Cartago", province: "Cartago", population: 147898, budget: 80000000 },
  { name: "Para\xEDso", province: "Cartago", population: 57543, budget: 35000000 },
  { name: "La Uni\xF3n", province: "Cartago", population: 99788, budget: 50000000 },
  { name: "Jim\xE9nez", province: "Cartago", population: 14507, budget: 15000000 },
  ...
  { name: "Bel\xE9n", province: "Heredia", population: 21333, budget: 20000000 },
  ...
  { name: "Sarapiqu\xED", province: "Heredia", population: 57247, budget: 35000000 },
  ...
  { name: "Ca\xF1as", province: "Guanacaste", population: 26570, budget: 20000000 },
  { name: "Abangares", province: "Guanacaste", population: 18039, budget: 15000000 },
  { name: "Tilar\xE1n", province: "Guanacaste", population: 19215, budget: 16000000 },
  ...
  { name: "Hojancha", province: "Guanacaste", population: 7478, budget: 1e7 },
  ...
  { name: "Monteverde", province: "Puntarenas", population: 6525, budget: 1e7 },
  { name: "Puerto Jim\xE9nez", province: "Puntarenas", population: 8721, budget: 11000000 },
  { name: "Lim\xF3n", province: "Lim\xF3n", population: 94001, budget: 55000000 },
  { name: "Pococ\xED", province: "Lim\xF3n", population: 128822, budget: 65000000 },
  { name: "Siquirres", province: "Lim\xF3n", population: 56645, budget: 38000000 },
  { name: "Talamanca", province: "Lim\xF3n", population: 30993, budget: 25000000 },
  { name: "Matina", province: "Lim\xF3n", population: 37562, budget: 28000000 },
...
class MunicipalEntity extends OutreachEntity {
  ...
  constructor(data = {}) {
    ...
    this.waveDate = data.waveDate ?? null;
    this.batchStatus = data.batchStatus ?? null;
    this.municipalityId = data.municipalityId ?? data.id ?? null;
    this.localGovernmentId = data.localGovernmentId ?? null;
    this.outreachVariant = data.outreachVariant ?? null;
    this.outreachSentAt = data.outreachSentAt ?? null;
    this.repliedAt = data.repliedAt ?? null;
    this.replySentiment = data.replySentiment ?? null;
    this.verifiedAt = data.verifiedAt ?? null;
    this.researchedAt = data.researchedAt ?? null;
    this.xHandle = data.xHandle ?? null;
    this.xUrl = data.xUrl ?? null;
    this.xWarmupPhase1At = data.xWarmupPhase1At ?? null;
    this.xWarmupPhase2At = data.xWarmupPhase2At ?? null;
    this.xEngagementCount = data.xEngagementCount ?? null;
    this.xLastEngagementAt = data.xLastEngagementAt ?? null;
  }
  getPrimaryContact() {
    if (this.typeData.decisionMakers && this.typeData.decisionMakers.length > 0) {
      const dm = this.typeData.decisionMakers.find((d) => d.influence === "high") || this.typeData.decisionMakers[0];
      return {
        name: dm.name,
        title: dm.title,
  ...
  matchesCriteria(criteria) {
    if (criteria.populationMin && this.typeData.population) {
      if (this.typeData.population < criteria.populationMin)
        return false;
    }
    if (criteria.populationMax && this.typeData.population) {
      if (this.typeData.population > criteria.populationMax)
        return false;
    }
  ...
  static fromJSON(data) {
    return new MunicipalEntity(data);
  }
  static fromCanton(canton) {
    return new MunicipalEntity({
      ...
      typeData: {
        ...
        departments: [
          { name: "Tecnolog\xEDa", focus: ["digital transformation"] },
          { name: "Planificaci\xF3n", focus: ["smart city"] }
        ],
...
// src/entities/VCEntity.ts
class VCEntity extends OutreachEntity {
  typeData;
  constructor(data = {}) {
    super({ ...data, type: "vc" });
    this.typeData = {
  ...
  matchesCriteria(criteria) {
    ...
    if (criteria.checkSizeMin && this.typeData.checkSizeMax) {
      if (this.typeData.checkSizeMax < criteria.checkSizeMin)
        return false;
    }
    if (criteria.checkSizeMax && this.typeData.checkSizeMin) {
      if (this.typeData.checkSizeMin > criteria.checkSizeMax)
        return false;
    }
    return true;
  }
...
class StrategyRegistry {
  ...
  getTypes() {
    return Array.from(this.strategies.keys()).filter((t) => t !== "default");
  }
  getActions(type) {
    if (this.strategies.has(type)) {
...
class MunicipalDiscoveryStrategy extends DiscoveryStrategy {
  ...
  async researchMunicipality(name, location = {}) {
    console.log(`\uD83D\uDCDA Researching municipality: ${name}...`);
    if (location.country === "Costa Rica") {
      return this.researchCostaRicaMunicipality(name, location);
    }
    return null;
  }
  async researchCostaRicaMunicipality(name, location) {
    ...
    return new MunicipalEntity({
      ...
      typeData: {
        ...
        province: canton.province,
        departments: [
          { name: "Tecnolog\xEDa de la Informaci\xF3n", focus: ["digital transformation", "e-government"] },
          { name: "Planificaci\xF3n Urbana", focus: ["smart city", "data analytics"] },
          { name: "Servicios Ciudadanos", focus: ["citizen engagement", "transparency"] }
        ],
        initiatives: [
          { name: "Transformaci\xF3n Digital Municipal", description: "Modernizaci\xF3n de servicios ciudadanos", status: "active", budget: canton.budget * 0.05 },
  ...
  generateMockMunicipals(query, limit) {
    const provinces = ["San Jos\xE9", "Alajuela", "Cartago", "Heredia", "Guanacaste", "Puntarenas", "Lim\xF3n"];
    const mockMunicipals = [];
    for (let i = 0;i < Math.min(limit, provinces.length); i++) {
      ...
      mockMunicipals.push(new MunicipalEntity({
        ...
        typeData: {
          ...
          budget: 20000000 + Math.floor(Math.random() * 1e8),
          province,
          departments: [
            { name: "Tecnolog\xEDa", focus: ["digital transformation"] },
            { name: "Planificaci\xF3n", focus: ["smart city"] }
...
class VCDiscoveryStrategy extends DiscoveryStrategy {
  searchQueries;
  constructor(config = {}) {
    super(config);
  ...
  scoreRelevance(vcData) {
    let score = 0;
    const focusText = (vcData.focusAreas || vcData.sectorFocus || []).join(" ").toLowerCase();
    ...
    const stageText = (vcData.stageFocus || []).join(" ").toLowerCase();
...
// src/strategies/personalization/PersonalizationStrategy.ts
...
// src/strategies/personalization/MunicipalPersonalizationStrategy.ts
class MunicipalPersonalizationStrategy extends PersonalizationStrategy {
  constructor(config = {}) {
    super(config);
  ...
  async personalize(entity) {
    ...
    try {
      const { generateEmail: generateEmail2 } = await Promise.resolve().then(() => (init_outreach_email_template(), exports_outreach_email_template));
      emailHtml = generateEmail2({
        recipientName,
        companyName,
  ...
  generateSubject(entity, companyName) {
    const topInitiative = entity.getTopInitiative();
    if (topInitiative) {
      return `Apoyando la transformaci\xF3n digital de ${companyName}`;
    }
  ...
  generateValueProposition(entity) {
    ...
    return `Alygn apoya a municipios como el suyo (${population.toLocaleString()} habitantes) en ${province} con la implementaci\xF3n pr\xE1ctica de gobernanza de IA.`;
  }
  async personalizeDryRun(entity) {
    ...
    const subject = this.generateSubject(municipalEntity, companyName);
    const painPoints = municipalEntity.personalizationContext?.painPoints || ["gobernanza de IA"];
    const mockEmail = {
      subject,
...
// src/strategies/personalization/VCPersonalizationStrategy.ts
class VCPersonalizationStrategy extends PersonalizationStrategy {
  databaseId;
  constructor(config = {}) {
    super(config);
  ...
  async personalize(entity) {
    ...
    let emailHtml = `<!-- Email template for ${entity.name} -->`;
    try {
      const { generateEmailHTML: generateEmailHTML2 } = await Promise.resolve().then(() => (init_outreach_email_template(), exports_outreach_email_template));
      emailHtml = generateEmailHTML2({
  ...
  generateVCSpecificHook(entity, portfolioCompanies, sectorFocus) {
  ...
  generateUniquePainPoints(entity, portfolioCompanies) {
    const firmName = entity.name;
    if (portfolioCompanies.length > 0) {
      const company = portfolioCompanies[0];
      return [
  ...
  generatePersonalizedPS(entity, partner, portfolioCompanies) {
    const psOptions = [];
    if (partner?.title) {
  ...
  generateSubject(entity, portfolioCompanies) {
    if (portfolioCompanies.length > 0) {
...
// src/strategies/research/ResearchStrategy.ts
...
class MunicipalResearchStrategy extends ResearchStrategy {
  constructor(config = {}) {
    super(config);
  ...
  researchCostaRica(entity) {
    ...
    entity.researchNotes = `Canton of ${canton.province} province. Population: ${canton.population}. Key initiatives: ${research.initiatives.map((i) => i.name).join(", ")}.`;
  ...
  async researchDryRun(entity) {
    console.log(`\uD83D\uDCDA [DRY RUN] Researching municipality: ${entity.name}...`);
    const mockResearch = {
      initiatives: [
        { name: "Digital Transformation", description: "Modernizing services", status: "active", budget: 1e5 }
      ],
      painPoints: ["AI accountability", "Digital transformation", "Citizen services"],
      trAigaRelevant: true,
  ...
  generateKeyContacts(municipalityName) {
    return [
      {
      ...
      {
        name: "IT Director",
        title: "Director de Tecnolog\xEDa",
...
class SendingStrategy {
  ...
  async initialize() {
    const providerType = this.config.providerType || "smtp";
    let providerConfig = this.config.providerConfig || {};
  ...
  async send(entity, options = {}) {
    if (!this.emailService) {
      await this.initialize();
    }
    const dryRun = options.dryRun || false;
    const draftStatus = options.draftStatus || "Approved";
    const sendToList = options.sendToList || [];
    ...
    const payload = {
      ...
      subject,
      html,
      from: this.config.fromEmail || "Alygn R&D <andrew@alygn.com>"
    };
    if (dryRun) {
  ...
  async checkAlreadySent(email, type) {
    try {
      const { SentEmailTracker: SentEmailTracker2 } = await Promise.resolve().then(() => (init_SentEmailTracker(), exports_SentEmailTracker));
      const tracker = new SentEmailTracker2;
...
class ValidationStrategy {
  ...
  async validateBatch(entities, options = {}) {
    const results = [];
...
class Pipeline {
  ...
  initializeStrategies() {
    this.registry.register("vc", "discover", new VCDiscoveryStrategy(this.config.discovery));
    this.registry.register("municipal", "discover", new MunicipalDiscoveryStrategy(this.config.discovery));
    this.registry.register("default", "validate", new ValidationStrategy(this.config.validation));
    this.registry.register("vc", "research", new VCResearchStrategy(this.config.research));
    this.registry.register("municipal", "research", new MunicipalResearchStrategy(this.config.research));
    this.registry.register("vc", "personalize", new VCPersonalizationStrategy(this.config.personalization));
    this.registry.register("municipal", "personalize", new MunicipalPersonalizationStrategy(this.config.personalization));
    this.registry.register("default", "send", new SendingStrategy(this.config.sending));
  }
...
\uD83D\uDE80 Running ${this.type.toUpperCase()} pipeline: ${action}`);
  ...
  createEntityFromData(data) {
    if (!data)
      return null;
    const entityType = data.type || this.type;
    if (entityType === "vc") {
  ...
  async runValidate(options) {
    ...
    if (dryRun) {
      ...
      const results2 = entities.map((e) => ({
        name: e.name,
        email: e.email,
        wouldValidate: true,
        result: e.email ? "valid" : "no_email"
      }));
      const stateFile2 = this.saveState("validated", {
    ...
    const strategy = this.registry.get("default", "validate");
    const results = [];
    for (const entity of entities) {
  ...
  async runPersonalize(options) {
    ...
    const strategy = this.registry.get(this.type, "personalize");
    const results = [];
    for (const entity of entities) {
      let result;
  ...
  async filterAlreadySent(entities) {
    const filtered = [];
    for (const entity of entities) {
  ...
  async runSend(options) {
    ...
    const strategy = this.registry.get("default", "send");
    const results = [];
    for (const entity of entities) {
...
`);
    ...
    results.stages.personalize = personalizeResult;
    results.summary.personalized = personalizeResult.personalized || 0;
    const sendResult = await this.runSend({ dryRun, limit });
    results.stages.send = sendResult;
    results.summary.sent = sendResult.sent || 0;
    const stateFile = this.saveState("pipeline", results);
    results.stateFile = stateFile;
    return results;
  }
}
