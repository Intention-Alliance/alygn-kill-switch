...
import { findConnectionNote } from './linkedin-notes';
 ...
 *
 * Uses the curated 25-note library (governance-first positioning,
 * andler-ops#160) when the primary partner matches; otherwise falls
 * back to the generic generator.
 */
export function generateLinkedInNote(entity: OutreachEntity): string {
  const vcEntity = entity as VCEntity;
  const partner = vcEntity.typeData?.partners?.[0];
  const curated = partner?.name ? findConnectionNote(partner.name) : null;
  if (curated) {
    return curated.note;
  }
  ...
  switch (method) {
    ...
    case 'linkedin':
      ...
      return {
        ...
        details: {
          linkedInUrl: vcEntity.getLinkedInUrl() || undefined,
          partnerName: vcEntity.typeData?.partners?.[0]?.name,
