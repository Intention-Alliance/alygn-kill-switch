...
async function postToTwitter(text) {
  // Format with signature + hashtag (MANDATORY)
  const formatted = formatTweet(text);
  ...
  try {
    ...
    // Initialize X API client with OAuth 1.0a (same as discovery system)
    const oauth1 = new OAuth1(creds);
    const client = new Client({ oauth1 });
    
    // Post tweet (using formatted text with signature + hashtag)
    const response = await client.posts.create({ text: formatted });
...
async function notifyWhatsApp(post, tweetId) {
  try {
    ...
    execSync(`openclaw message send -t "+50662163355" --channel whatsapp -m "${message.replace(/"/g, '\\"').replace(/\n/g, '\\n')}"`, {
      cwd: WORKSPACE,
      stdio: 'inherit'
    });
