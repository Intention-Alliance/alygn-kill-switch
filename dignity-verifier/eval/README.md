...
bun dignity-verifier/eval/run-eval.ts

# Just the original 13-test baseline
bun dignity-verifier/eval/run-eval.ts --set original

# Just the held-out 20-test set
bun dignity-verifier/eval/run-eval.ts --set heldout

# Different model
bun dignity-verifier/eval/run-eval.ts --model qwen2.5:0.5B

# Cap the number of tests (useful for slow smoke runs)
bun dignity-verifier/eval/run-eval.ts --set original --limit 5
```
...
```bash
bun test dignity-verifier/eval/
```
...
`dignity-verifier/.gitignore`). The v1 eval data and manifest are immutable
