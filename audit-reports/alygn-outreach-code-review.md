...
this.typeData = {
  governmentType:
    data.typeData?.governmentType || data.governmentType || "city",
...
```markdown
Pipeline state is persisted to `$HOME/.openclaw/workspace/reports/alygn/{subFolderType}/alygn-{type}-{phase}-{date}.json`
```
...
```typescript
recentInvestments: data.recentInvestments.map((c) => ({
  company: c,
  date: "2024-01",
  stage: "Seed",
}));
...
```typescript
const { SentEmailTracker } = await import(
  `${process.env.HOME}/.openclaw/workspace/scripts/alygn/lib/SentEmailTracker.js`
);
