...
describe('Slashing Webhook Tests', () => {
  ...
  it.skipIf(SKIP_WEBHOOK_TESTS)('returns slashing rules', async () => {
    const response = await fetch(`${SLASHING_ENGINE_URL}/api/slashing/rules`);
...
if (SKIP_WEBHOOK_TESTS) {
  console.log('⚠️ Webhook tests skipped - start slashing engine and unset SKIP_WEBHOOK_TESTS');
}
